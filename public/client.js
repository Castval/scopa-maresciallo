...
socket.on('richiestaAmicizia', ({ da }) => { mostraMessaggio(`${da} ti ha inviato una richiesta`,'info'); if(Notification.permission==='granted'&&document.hidden) try{new Notification('Scopa Maresciallo',{body:`${da} ti ha inviato una richiesta`});}catch(e){} caricaAmici(); });
...
document.getElementById('btnCreaTorneo')?.addEventListener('click', async () => {
  ...
  const d = await (await fetch('/api/admin/torneo/crea', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ admin: getNomeUtente(), nome, numGiocatori: num, modalitaVittoria: mod, valoreVittoria: parseInt(val), controlloIp: ip }) })).json();
